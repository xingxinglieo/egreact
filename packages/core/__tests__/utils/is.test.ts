...
describe('is', () => {
  ...
  it('should tell me if something IS an boolean', () => {
    expect(is.boo(myFunc)).toBe(false)
    expect(is.boo(myArr)).toBe(false)
    expect(is.boo(myUnd)).toBe(false)

    expect(is.boo(myBoo)).toBe(true)
  })
